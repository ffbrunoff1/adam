  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled || isOpen ? 'bg-primary shadow-md' : 'bg-transparent'
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="md:hidden absolute top-full left-0 w-full bg-primary shadow-lg pb-6"
            ...
            >
              {navLinks.map(link => (
                <motion.a
                  ...
                  className="text-lg text-secondary hover:text-accent transition-colors duration-300"
